...
class Repository {
    ...
    constructor(repository, repositoryResolver, pushErrorHandlerRegistry, remoteSourcePublisherRegistry, postCommitCommandsProviderRegistry, branchProtectionProviderRegistry, historyItemDetailProviderRegistry, globalState, logger, telemetryReporter) {
        ...
        this._sourceControl = vscode_1.scm.createSourceControl('git', 'Git', root, parent);
    ...
    async worktree(options) {
        await this.run(operation_1.Operation.Worktree, () => this.repository.worktree(options));
    }
    async deleteWorktree(path) {
        await this.run(operation_1.Operation.DeleteWorktree, () => this.repository.deleteWorktree(path));
    }
