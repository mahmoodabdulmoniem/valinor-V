...
class CommandCenter {
    ...
    async _createWorktree(repository, worktreePath, name) {
        const config = vscode_1.workspace.getConfiguration('git');
        const showRefDetails = config.get('showReferenceDetails') === true;
        if (!name) {
            const getBranchPicks = async () => {
                ...
                const processors = [new RefProcessor(0 /* RefType.Head */, BranchItem)];
                const itemsProcessor = new RefItemsProcessor(repository, processors);
                return itemsProcessor.processRefs(refs);
            };
            const placeHolder = vscode_1.l10n.t('Select a branch to create the new worktree from');
            const choice = await this.pickRef(getBranchPicks(), placeHolder);
        ...
        const disposables = [];
        const inputBox = vscode_1.window.createInputBox();
        inputBox.placeholder = vscode_1.l10n.t('Worktree name');
        inputBox.prompt = vscode_1.l10n.t('Please provide a worktree name');
        inputBox.value = name || '';
        inputBox.show();
        const worktreeName = await new Promise((resolve) => {
            disposables.push(inputBox.onDidHide(() => resolve(undefined)));
            disposables.push(inputBox.onDidAccept(() => resolve(inputBox.value)));
        });
        (0, util_1.dispose)(disposables);
        ...
        const defaultUri = vscode_1.Uri.file(path.dirname(repository.root));
        const uris = await vscode_1.window.showOpenDialog({
            defaultUri,
            canSelectFiles: false,
            canSelectFolders: true,
