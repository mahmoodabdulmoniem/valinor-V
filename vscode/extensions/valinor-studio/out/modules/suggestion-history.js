...
exports.registerSuggestionHistory = exports.SuggestionHistoryProvider = void 0;
...
class SuggestionHistoryProvider {
    ...
    async filterHistory(filter) {
        ...
        try {
            ...
            if (filter.section) {
                filtered = filtered.filter(item => item.section.toLowerCase().includes(filter.section.toLowerCase()));
            }
            if (filter.model) {
                filtered = filtered.filter(item => item.model.toLowerCase().includes(filter.model.toLowerCase()));
            }
            if (filter.dateRange) {
    ...
    async openDiffEditor(suggestion) {
        try {
            ...
            await vscode.commands.executeCommand('vscode.diff', originalUri, suggestedUri, `Suggestion History - ${suggestion.section} (${suggestion.model})`, { preview: true });
            // Clean up temp files after a delay
            setTimeout(() => {
                try {
                    if (fs.existsSync(originalFile))
                        fs.unlinkSync(originalFile);
                    if (fs.existsSync(suggestedFile))
                        fs.unlinkSync(suggestedFile);
                }
                catch (error) {
...
exports.SuggestionHistoryProvider = SuggestionHistoryProvider;
SuggestionHistoryProvider.viewType = 'valinorStudio.suggestionHistory';
function registerSuggestionHistory(context) {
    const provider = new SuggestionHistoryProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(SuggestionHistoryProvider.viewType, provider));
    // Register command to open suggestion history
    context.subscriptions.push(vscode.commands.registerCommand('valinorStudio.openSuggestionHistory', () => {
        vscode.commands.executeCommand('valinorStudio.suggestionHistory.focus');
    }));
    return provider;
}
exports.registerSuggestionHistory = registerSuggestionHistory;
//# sourceMappingURL=suggestion-history.js.map