...
import { Comment, CommentReply, CollaborationManager } from './collaboration';

export class CommentPanelProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = 'valinorStudio.commentPanel';
	...
	private async handleAddComment(content: string, severity: string): Promise<void> {
		...
		try {
			await this._collaborationManager.addComment(editor, content, severity as any);
			this.refreshComments();
	...
	private async handleReplyToComment(commentId: string, content: string): Promise<void> {
		try {
	...
	private async handleNavigateToComment(commentId: string): Promise<void> {
