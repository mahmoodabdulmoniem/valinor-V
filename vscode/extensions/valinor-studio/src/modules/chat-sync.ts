...
export class ChatSync {
	...
	private _chatProvider: any; // Reference to chat provider

	constructor(chatProvider: any) {
		this._chatProvider = chatProvider;
		this.initializeChatSync();
	}

	private initializeChatSync(): void {
	...
	private detectCurrentSection(document: vscode.TextDocument, lineNumber: number): string {
	...
	private detectCurrentContext(editor: vscode.TextEditor, position: vscode.Position): string {
		const document = editor.document;
	...
	public async askAboutCurrentSection(): Promise<void> {
	...
	public async askAboutSelectedText(): Promise<void> {
		const editor = vscode.window.activeTextEditor;
		if (!editor) {
	...
	public async suggestImprovements(): Promise<void> {
		const editor = vscode.window.activeTextEditor;
		if (!editor) {
			vscode.window.showWarningMessage('No active editor');
			return;
	...
	private getSectionContent(document: vscode.TextDocument, lineNumber: number): string {
		const lines: string[] = [];
