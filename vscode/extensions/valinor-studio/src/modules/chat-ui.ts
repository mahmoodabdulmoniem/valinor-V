...
import { ChatIntelligence, ConversationResponse, RFPData } from './chat-intelligence';
import { ChatActionProvider, AutoFix, InChatCreation, SleekIcons } from './chat-actions';
...
export class ValinorChatViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = 'valinorStudio.chat';
	private _view?: vscode.WebviewView;
	private _messages: ChatMessage[] = [];
	private _selectedModel: string = 'GPT-4';
	private _currentRFPData: any = null;
	private _output: vscode.OutputChannel;
	private _chatIntelligence: ChatIntelligence;
	private _isProcessing: boolean = false;
	private _actionProvider: ChatActionProvider;

	constructor(private readonly _extensionUri: vscode.Uri, output: vscode.OutputChannel) {
		this._output = output;
		this._chatIntelligence = new ChatIntelligence(output);
		this._actionProvider = new ChatActionProvider(output);
	}
	...
	public addMessage(role: 'user' | 'ai', text: string, model?: string, sources?: SourceLink[], actions?: MessageAction[], suggestions?: string[], context?: any) {
	...
	private getActionIcon(actionType: string): string {
		const iconMap: { [key: string]: string } = {
			'insert': SleekIcons.INSERT,
			'generate': SleekIcons.GENERATE,
			'analyze': SleekIcons.ANALYZE,
			'search': SleekIcons.ANALYZE,
			'explain': SleekIcons.EXPLAIN,
			'suggest': SleekIcons.LIGHTBULB,
			'navigate': SleekIcons.OPEN,
			'improve': SleekIcons.IMPROVE,
			'create': SleekIcons.CREATE,
			'fix': SleekIcons.FIX,
			'refactor': SleekIcons.REFACTOR
		};
		return iconMap[actionType] || SleekIcons.ROCKET;
	}
	...
	private async generateContextualResponse(userQuestion: string): Promise<{ text: string; sources: SourceLink[]; actions: MessageAction[] }> {
...
Note: No RFP data is currently loaded. Please use the "Import RFP" command to load contract data for contextual responses.`;
		...
		this.addMessage = (type: string, content: string, model?: string) => {
			if (type === 'ai') {
				aiResponse = content;
			}
		};
		...
		try {
			await analyzeContractWithAI(mockContractData, output, this, this._selectedModel);
		} finally {
	...
	private isDocumentInsertionRelevant(question: string): boolean {
...
I'm here to help you analyze government contracts and create compelling proposals!`;
	...
	private handleInsertIntoDocument(messageId: string, section?: string) {
	...
	private async insertContentIntoDocument(content: string, section: string) {
		try {
...
`;
	...
	private handleNavigateToSource(source: SourceLink) {
		if (source.filePath) {
	...
	private async handleShowInChatCreations() {
		const context = {
	...
	private formatAutoFixesMessage(fixes: AutoFix[]): string {
		let message = `## ${SleekIcons.FIX} Auto-Fixes Available\n\n`;
	...
	private formatInChatCreationsMessage(creations: InChatCreation[]): string {
		let message = `## ${SleekIcons.CREATE} In-Chat Creation Options\n\n`;

		creations.forEach((creation, index) => {
			message += `### ${SleekIcons.SECTION} ${creation.name}\n`;
