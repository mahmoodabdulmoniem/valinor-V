...
/**
 * Corrected test script for enhanced SAM API functionality
 * Shows the real behavior of the SAM.gov API
 */
...
// Corrected SAM.gov API search with proper validation
async function searchSAMGovAPI(noticeId) {
	...
	const startDate = new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000); // 30 days back
	const endDate = new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000); // 30 days forward

	// Strategy 1: Try exact solicitation number search (with validation)
		...
		for (const pattern of analysis.patterns) {
			try {
				...
				if (data.opportunitiesData && data.opportunitiesData.length > 0) {
					...
					// The SAM.gov API often ignores the solicitationNumber parameter
					...
					console.log(`   This is a known bug in the SAM.gov API`);
				}
			} catch (error) {
	...
	// Strategy 3: Broad search with pattern matching (MOST RELIABLE)
	try {
		console.log(`\n🔎 Trying broad search with pattern matching (MOST RELIABLE)`);
		...
		if (data.opportunitiesData && data.opportunitiesData.length > 0) {
			...
			console.log(`❌ No exact matches found in broad search`);
			console.log(`   Available solicitation numbers: ${data.opportunitiesData.slice(0, 10).map(c => c.solicitationNumber).join(', ')}`);
		}
	} catch (error) {
...
async function runTests() {
	console.log('🚀 Testing Enhanced SAM API Functionality (Corrected Version)\n');
	console.log('='.repeat(60));

	const testCases = [
		// Known working examples
		'SPE4A725R0357',  // This should work
	...
	console.log('\n🔍 Key Findings:');
	console.log('❌ SAM.gov API solicitationNumber parameter is BROKEN');
	console.log('❌ API often ignores search parameters and returns random results');
	console.log('✅ API Notice ID searches work reliably');
	console.log('✅ Broad search with client-side filtering is most reliable');
	console.log('\n💡 Solution:');
	console.log('✅ Enhanced SAM API now validates all responses');
	console.log('✅ Multiple search strategies with proper validation');
	console.log('✅ Clear error messages when API parameters are ignored');
	console.log('✅ Fallback to broad search when direct search fails');
}
